...
results = os.getcwd()[:-(len('src/getHaplotype/'))] + "results/"
...
for index, row in todo.iterrows():
	gene = row['gene']
	fileCheck = Path(results + gene) 
	if fileCheck.is_dir():
		fileCheck = Path(results + gene + "/meta_" + gene + ".vcf") 
		if (fileCheck.is_file()): # if have meta file, do this block
			#check if have full length haplos
			temp = pd.read_csv(results + gene + "/full_length_haplotypes_" + gene + ".vcf", sep="\t")
